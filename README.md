...
- [Free Animation Power Desktop](https://github.com/freeanimationpower/FAP_PC_WEB_VERSION) — Estudio de animación 2D para PC
...
flowchart LR
    ...
    classDef ext fill:#ffffff,stroke:#1a1a1a,color:#1a1a1a,stroke-width:2px;
        ...
        FMT --> PRM["Constructor de prompt<br/>estructura profesional forzada<br/>+ parámetros de tono y extension"]
        PRM --> API["API de IA configurable<br/>trae tu propia key"]
        API --> EDT["Editor de guion<br/>edición libre por escenas"]
        EDT --> FIX["Corrección inteligente<br/>por notas del director o 2a IA"]
...
- **Proyectos guardados** en el navegador (hasta 30, con carga completa de parámetros).
- **Exportación**: copiar al portapapeles, `.txt`, `.fountain` (compatible con Final Draft / Highland 2 / Trelby), `.docx` (Word real) y `PDF` con portada, marca de agua y formato de guion profesional.
...
# 5. Escribir idea, elegir formato y parámetros
...
Ver sección [Corrección inteligente](#corrección-inteligente).

### 5. Exportar

Ver sección [Exportación](#exportación).
...
| `ACTO IV` | soporta números romanos |
| `ESCENA 5 DIALOGO` | ignora las palabras extra y corrige la escena 5 |
| `TODO` | corrige el guion completo |

La app divide el guion en bloques detectando encabezados (`ESCENA`, `SECUENCIA`, `SECCION`, `BLOQUE`, `ACTO`, `MISION`, `BEAT`, `SHOT`, `COLD OPEN`, `EPISODIO`) con número arabigo o romano.
...
- Cada corrección guarda: instrucción, alcance, texto antes, texto después y fecha.
- **Deshacer última corrección** restaura el estado anterior si el guion no fue editado manualmente después.
...
| **Word** | `.docx` real | generado en el navegador (JSZip CDN) con formato profesional. Fallback a `.doc` compatible si no hay conexión |
...
- **Portada**: logo Free Animation Power, "SCRIPT AI", título del guion, fecha y los logos con **freeanimationpower.org** y **fierroduque.com** al pie.
- **Marca de agua**: en cada página, al pie, los logos y URLs semitransparentes (opacidad 7%).
- El Word incluye su propia portada con salto de página.
...
- **Exportación**: jsPDF + JSZip (CDN) — empaquetado 100% en el navegador.
...
- **Reintentos inteligentes**: 429 (rate limit) reintenta 2 veces esperando 30s/50s; 503 (modelo cargando) esperando 50s/80s; respuesta sin texto parseable reintenta 3 veces; errores 4xx fallan rápido sin reintentar.
...
- Anti-DoS: body máximo 4 MB, key máximo 512 caracteres sin saltos de linea.
...
Wrapper `lsSet()` con limpieza de emergencia ante `QuotaExceededError` (código 22 / Firefox 1014):
...
| `HEARTBEAT_MS` | `15000` | Intervalo del latido de conexión en ms |
...
- Los CDN (Tailwind, jsPDF, JSZip) requieren conexión a internet del navegador; el resto funciona offline.
...
Secciones principales de `app.js`: `I18N` (ES/EN) · `TEXT_PROVIDERS` (11 conectores) · `SCRIPT_FORMATS` (12 plantillas de estructura) · `lsSet` (almacenamiento seguro) · `cleanLLMResponse` (anti-chattiness) · `callTextAPI` (proxy + reintentos) · `parseSceneRange` / `extractSceneBlocks` (corrección por escenas) · `classifyScriptLines` (formato profesional de export) · `exportPdf` / `exportWord` / `exportTxt` / `exportFountain`.
